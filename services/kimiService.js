...
请严格按以下JSON格式返回（不要包含其他文字，不要添加markdown代码块标记）：
{
  "overallScore": 83,
  "overallLabel": "相对匹配",
  "overallComment": "比及格好了一点，但离理想状态还有距离",
  "dimensions": [
    {"name": "AIGC工具", "score": 85, "fullMark": 100},
    {"name": "影像编辑", "score": 80, "fullMark": 100},
    {"name": "工作流搭建", "score": 70, "fullMark": 100},
    {"name": "审美创意", "score": 75, "fullMark": 100},
    {"name": "品牌思维", "score": 65, "fullMark": 100},
    {"name": "导演思维", "score": 70, "fullMark": 100}
  ],
  "strengths": [
    {
      "title": "AIGC工具掌握不错",
      "description": "你熟悉Midjourney等AI工具的使用，能够生成符合需求的视觉内容，这是你的核心竞争力。",
      "score": 85
    }
  ],
  "weaknesses": [
    {
      "title": "品牌思维需要加强",
      "description": "你的作品技术层面不错，但在品牌调性把握上还有提升空间，建议多研究成功案例。",
      "score": 65
    }
  ],
  "actions": [
...
3. 行动建议要具体、可执行
4. strengths和weaknesses至少各要有1条，描述至少50个字
...
async function analyzeJobDescription(jobDescription) {
  ...
  try {
    ...
    console.error('岗位解析JSON失败:', e.message, '原始结果:', result.substring(0, 200));
    // 返回默认结构
    return {
      title: "未知岗位",
      company: "",
...
async function analyzeResume(resumeText) {
  ...
  const messages = [
    { role: 'system', content: '你是一位专业的人力资源专家，擅长分析简历并提取关键信息。返回JSON格式。' },
...
async function analyzeMatch(resumeAnalysis, jobAnalysis) {
  ...
  const messages = [
    { role: 'system', content: '你是一位资深的职业规划师，擅长分析候选人与岗位的匹配度，给出客观、有建设性的反馈。返回JSON格式。' },
  ...
  console.log('Kimi原始返回:', result.substring(0, 500));
  console.log('清理后:', cleanedResult.substring(0, 500));
  ...
  try {
    ...
    if (!parsed.weaknesses || parsed.weaknesses.length === 0) {
      parsed.weaknesses = [{
        title: "还有提升空间",
        description: "与岗位要求相比，你在某些方面还有差距。建议针对性地学习提升，弥补短板，增强竞争力。",
        score: 60
      }];
    }
    ...
    return {
      overallScore: 70,
      overallLabel: "勉强匹配",
      overallComment: "比及格好了一点，但还有提升空间",
      dimensions: [
        {name: "AIGC工具", score: 70, fullMark: 100},
        {name: "影像编辑", score: 70, fullMark: 100},
        {name: "工作流搭建", score: 70, fullMark: 100},
